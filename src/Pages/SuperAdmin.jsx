...
export default function SuperAdmin() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      let response;
      if (type === "departments") {
        response = await api.put(`/departments/${data.old_name}`, {
          dept_name: data.dept_name,
        });
        setDepartments(
          departments.map((d) =>
            d.dept_name === data.old_name ? response.data : d
          )
        );
      } else if (type === "batches") {
        ...
        setBatches(
          batches.map((b) => (b.batch_id === data.batch_id ? response.data : b))
        );
      } else if (type === "sections") {
        ...
        setSections(
          sections.map((s) =>
            s.section_id === data.section_id ? response.data : s
          )
        );
      } else if (type === "subjects") {
        ...
        setSubjects(
          subjects.map((s) =>
            s.subject_code === data.old_code ? response.data : s
          )
        );
      } else if (type === "students") {
        ...
        setStudents(
          students.map((s) =>
            s.register_number === data.old_reg_num ? response.data : s
          )
        );
  ...
  const handleDelete = async (type, id, e) => {
    ...
    try {
      ...
      if (type === "departments")
        setDepartments(departments.filter((d) => d.dept_name !== id));
      if (type === "batches")
        setBatches(batches.filter((b) => b.batch_id !== id));
      if (type === "sections")
        setSections(sections.filter((s) => s.section_id !== id));
      if (type === "subjects")
        setSubjects(subjects.filter((s) => s.subject_code !== id));
      if (type === "students")
        setStudents(students.filter((s) => s.register_number !== id));
      if (type === "time-blocks")
        setTimeBlocks(timeBlocks.filter((b) => b.time_block_id !== id));
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg max-w-4xl mx-auto">
          {activeTab === "departments" && (
            <>
              <form
                onSubmit={(e) =>
                  handleAdd(e, "departments", { dept_name: deptName })
                }
                className="mb-6"
              >
                ...
                <div className="mt-4 text-sm text-gray-600">
                  ...
                  <ul className="list-disc pl-5">
                    <li>
                      ...
                      <code>RegisterNumber</code>, <code>Name</code>, and{" "}
                      <code>Section</code>.
                    </li>
                    <li>
                      Example:
                      <pre className="bg-gray-200 p-2 rounded mt-1">
                        RegisterNumber,Name,Section
                        <br />
                        CS1A001,Alice,A
                        <br />
                        CS1A002,Bob,B
                    ...
                    <li>
                      <code>Section</code> must match existing section names for
                      the selected department and year.
                    </li>
                    <li>
                      Existing students with matching{" "}
                      <code>RegisterNumber</code> will be updated; new ones will
                      be added.
                    </li>
                  </ul>
                </div>
                  ...
                  >
                    ...
                    {!collapsedDepts[dept] &&
                      Object.entries(years).map(([year, sections]) => (
                        <div
                          key={year}
                          className="border-l-4 border-blue-100 pl-4 mt-4"
                        >
                          ...
                          {Object.entries(sections).map(([section, studs]) => {
                            ...
                            return (
                              <div
                                key={section}
                                className="border-l-4 border-blue-200 pl-4 mt-2"
                              >
                                <div
                                  className="flex justify-between items-center cursor-pointer"
                                  onClick={() =>
                                    toggleSection(dept, year, section)
                                  }
                                >
                                        ...
                                        <div className="flex gap-2">
                                          <button
                                            onClick={() =>
                                              handleEdit("students", {
                                                ...st,
                                                old_reg_num: st.register_number,
                                              })
                                            }
                                            disabled={loading}
