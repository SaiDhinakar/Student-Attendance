...
DEFAULT_MODEL_PATH = "backend/checkpoints/LightCNN_29Layers_V2_checkpoint.pth.tar"
DEFAULT_YOLO_PATH = "backend/yolo/weights/yolo11n-face.pt"
BASE_DATA_DIR = "Student_image_data"
BASE_GALLERY_DIR = "gallery"
...
app = FastAPI(title="Face Recognition Gallery Manager", 
              ...
              docs_url=False, redoc_url=False, openapi_url=False)

# Mount static files
app.mount("/static", StaticFiles(directory="gallery_manager_app/static"), name="static")
...
SERVER_PORT = os.getenv("GALLERY_SERVER_PORT", "5001") 
...
@app.get("/gallery-manager", response_class=FileResponse)
async def serve_spa():
    return FileResponse("gallery_manager_app/static/index.html")

@app.get("gallery-manager/batches", summary="Get available batch years and departments")
...
@app.get("gallery-manager/galleries", summary="Get all available galleries")
async def list_galleries():
    """List all available face recognition galleries"""
    
    if not os.path.exists(BASE_GALLERY_DIR):
        return {"galleries": [], "path":BASE_GALLERY_DIR}
...
@app.get("gallery-manager/galleries/{year}/{department}", response_model=Optional[GalleryInfo], 
...
@app.post("gallery-manager/process", response_model=ProcessingResult, 
...
@app.post("gallery-manager/galleries/create", 
...
@app.post("gallery-manager/batches/year", status_code=201, summary="Add a new batch year")
...
@app.delete("gallery-manager/batches/year/{year}", status_code=200, summary="Delete a batch year")
...
@app.post("gallery-manager/batches/department", status_code=201, summary="Add a new department")
...
@app.delete("gallery-manager/batches/department/{department}", status_code=200, summary="Delete a department")
...
@app.get("gallery-manager/check-directories", summary="Check if directories exist and are accessible")
...
@app.post("gallery-manager/recognize", summary="Recognize faces in an uploaded image")
